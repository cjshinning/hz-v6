...
Page.include({
    ...
    resetSize: function() {
        ...
        _this.timer = setTimeout(function(){
            ...
            $topIcon.css("margin-right",$win.width()-1024);
            _this.resetScroll();
...
Search.include({
    ...
    showTopIcon:function(){
        ...
        if(this.$topIcon.length === 0){
            ...
            this.$topIcon.css({"margin-right":$win.width()-1003,"margin-bottom":3});
        }else{
    ...
    isGameMatched:function(gameObj){
        ...
        if(this.filterKeys["GAME_THEME"]){
            var r = false;
            if(!(gameObj["GAME_THEME"] instanceof Array)){
                return false;
            }
            for(var i = 0,len=gameObj["GAME_THEME"].length;i<len;i++){
                if(gameObj["GAME_THEME"][i] == this.filterKeys["GAME_THEME"]){
                    r = true;
                }
            }
            if(!r){
